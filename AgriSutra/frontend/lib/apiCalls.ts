...
export const fetchEquipmentByCategory = async (category: string) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/equipment/${category}`);
...
export const fetchAvailableEquipment = async (
  ...
  try {
    let url = `${API_BASE_URL}/api/equipment/available`;
    const params = new URLSearchParams();

    if (category) params.append("category", category.toLowerCase());
    if (availability && availability !== "All")
      params.append("availability", availability.toLowerCase());
...
export const fetchEquipmentDetails = async (equipment_id: string) => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/bookEquipment/${equipment_id}`,
    );
...
export const bookEquipment = async (
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3001/api/bookEquipment/${equipment_id}/book`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
