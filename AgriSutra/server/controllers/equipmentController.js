...
const getEquipmentByCategory = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ message: `No equipment found for category: ${category}` });
    }
...
const addEquipment = async (req, res) => {
  try {
    ...
    if (!req.body || Object.keys(req.body).length === 0) {
      return res
        .status(400)
        .json({ error: "Request body is empty. Please send valid data." });
    }
    ...
    if (req.user && req.user.id) {
      ...
      return res
        .status(401)
        .json({ error: "Owner ID is missing. User not authenticated." });
    }
...
const getAvailableEquipment = async (req, res) => {
  try {
    ...
    if (availability) {
      conditions.push(`
            LOWER(e.availability) = LOWER($$ { params.length + 1 })
            `);
...
const bookEquipment = async (req, res) => {
  ...
  try {
    ...
    const equipmentQuery = `
            SELECT owner_id, price, availability FROM equipment WHERE equipment_id = $1;
            `;
    ...
    const updateEquipmentQuery = `
            UPDATE equipment SET availability = 'Not Available'
